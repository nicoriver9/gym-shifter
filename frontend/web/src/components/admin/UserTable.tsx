...
const UserTable = () => {
  ...
  const toggleSortByName = () => {
    const sorted = [...filteredUsers].sort((a, b) => {
      ...
      return isAscOrder ? nameA.localeCompare(nameB) : nameB.localeCompare(nameA);
    });
  ...
  return (
    <div className="max-w-screen-2xl mx-auto mt-8 px-4 md:px-8">
        ...
        <table className="w-full table-fixed text-white text-sm">
          ...
          <tbody>
            {paginatedUsers.map((u) => (
              ...
              >
                <td className="px-2 py-3 font-bold break-words">{u.username}</td>
                <td className="px-2 py-3 break-words">{u.firstName} {u.lastName}</td>
                ...
                <td className="px-2 py-3 text-xs text-center align-top max-w-[140px] break-words">
                  {u.current_pack ? (
                    <div className="flex flex-col gap-0.5 leading-snug">
                      <span className="font-semibold text-purple-400">{u.current_pack.name}</span>
                      <span className="text-gray-300">
                      <div className="text-gray-300"><strong>📅</strong> {new Date(new Date(u.pack_expiration_date).setMonth(new Date(u.pack_expiration_date).getMonth() - 1)).toLocaleDateString()}</div>
                      </span>
                      {daysRemaining(u.pack_expiration_date) === 0 ? (
                        <span className="text-red-400 font-medium">⚠️ Pack vencido</span>
                      ) : (
                        <>
                          <span className="text-green-400">
                            <strong>🗓️</strong> {daysRemaining(u.pack_expiration_date)} días restantes
                          </span>
                          <span className="text-blue-300">
                            <strong>🎯</strong> {u.classes_remaining ?? 0} clases restantes
                          </span>
                        </>
                      )}
                    </div>
      ...
      <div className="md:hidden flex flex-col gap-4 px-2">
        {paginatedUsers.map((u) => (
          <div key={u.id} className="bg-gray-800 p-4 rounded-lg shadow-md text-white text-sm space-y-2" data-aos="fade-up" data-aos-duration="1000">
            <div className="font-bold text-lg text-purple-400">{u.firstName} {u.lastName}</div>
            ...
            <div>
              ...
              {u.current_pack ? (
                <div className="ml-2 space-y-1">
                  <div className="text-purple-300 font-medium">{u.current_pack.name}</div>
                  <div className="text-gray-300"><strong>📅</strong> {new Date(new Date(u.pack_expiration_date).setMonth(new Date(u.pack_expiration_date).getMonth() - 1)).toLocaleDateString()}</div>
                  {daysRemaining(u.pack_expiration_date) === 0 ? (
                    <div className="text-red-400 font-medium">⚠️ Pack vencido</div>
                  ) : (
                    <>
                      <div className="text-green-400"><strong>🗓️</strong> {daysRemaining(u.pack_expiration_date)} días restantes</div>
                      <div className="text-blue-300"><strong>🎯</strong> {u.classes_remaining ?? 0} clases restantes</div>
                    </>
                  )}
                </div>
              ) : (
